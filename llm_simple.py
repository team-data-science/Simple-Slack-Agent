...
# A very simple robot emojy that will go before every llm answer in Slack
def add_robot(text: str) -> str:
    return f"🤖 {text}"

# RunnableSequence that takes prompt and the llm. This seems is overkill, 
# because it's just one step. That's why I added the add_robot transformation here.
_chain = RunnableSequence(_prompt, _llm, add_robot)
